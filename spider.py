        ...
        if not self.submonitor_checknow:
            ...
            if "submonitor_dic" in submonitorconfig:
                ...
                if self.submonitor_live_cnt > 0 or self.submonitor_cnt > 0:
                    printlog(
                        '[Check] "%s" 子线程运行情况：%s/%s'
                        % (self.name, self.submonitor_live_cnt, self.submonitor_cnt)
                    )
            self.submonitor_checknow = False
...
class YoutubeLive(Monitor):
        ...
        while not self.stop_now:
            ...
            try:
                ...
                if self.is_firstrun:
                    writelog(
                        self.logpath,
                        '[Info] "%s" getyoutubevideodic %s: %s'
                        % (self.name, self.tgt, videodic_new),
                    )
                    self.is_firstrun = False
                writelog(
                    self.logpath,
                    '[Success] "%s" getyoutubevideodic %s' % (self.name, self.tgt),
                )
            except Exception as e:
                printlog(
                    '[Error] "%s" getyoutubevideodic %s: %s' % (self.name, self.tgt, e)
                )
                writelog(
                    self.logpath,
                    '[Error] "%s" getyoutubevideodic %s: %s' % (self.name, self.tgt, e),
            ...
            for video_id in self.videodic:
                if (
                    ...
                    try:
                        video_status = getyoutubevideostatus(
                        ...
                        writelog(
                            self.logpath,
                            '[Success] "%s" getyoutubevideostatus %s'
                            % (self.name, video_id),
                        )
                    except Exception as e:
                        printlog(
                            "[Error] %s getvideostatus %s: %s"
                            % (self.name, video_id, e)
                        )
                        writelog(
                            self.logpath,
                            '[Error] "%s" getyoutubevideostatus %s: %s'
                            % (self.name, video_id, e),
                        )
            time.sleep(self.interval)
    ...
    def push(self, video_id):
        if self.status_push.count(self.videodic[video_id]["video_status"]):
            # 获取视频简介
            try:
                video_description = getyoutubevideodescription(
                    video_id, self.cookies, self.proxy
                )
                writelog(
                    self.logpath,
                    '[Success] "%s" getyoutubevideodescription %s'
                    % (self.name, video_id),
                )
            except Exception as e:
                printlog(
                    '[Error] "%s" getyoutubevideodescription %s: %s'
                    % (self.name, video_id, e)
                )
                writelog(
                    self.logpath,
            ...
            if pushcolor_dic:
                pushtext = (
                    ...
                    % (
                        ...
                        self.tgt_name,
                        self.videodic[video_id]["video_type"],
                        self.videodic[video_id]["video_status"],
                        self.videodic[video_id]["video_title"],
                        waittime(self.videodic[video_id]["video_timestamp"]),
                        video_id,
                    )
                )
        ...
        if self.no_chat != "True":
            ...
            if (
                ...
                if (
                    ...
                    self.checksubmonitor()
                    printlog(
                        '[Info] "%s" startsubmonitor %s' % (self.name, monitor_name)
                    )
                    writelog(
                        self.logpath,
                        '[Info] "%s" startsubmonitor %s' % (self.name, monitor_name),
            ...
            # 停止记录弹幕
                ...
                if (
                    ...
                    self.checksubmonitor()
                    printlog(
                        '[Info] "%s" stopsubmonitor %s' % (self.name, monitor_name)
                    )
                    writelog(
                        self.logpath,
                        '[Info] "%s" stopsubmonitor %s' % (self.name, monitor_name),
...
class YoutubeChat(SubMonitor):
        ...
        while not self.stop_now:
            ...
            if not self.continuation:
                try:
                    ...
                    writelog(
                        self.logpath,
                        '[Info] "%s" getyoutubechatcontinuation %s: %s'
                        % (self.name, self.tgt, self.continuation),
                    )
                    writelog(
                        self.logpath,
                        '[Success] "%s" getyoutubechatcontinuation %s'
                        % (self.name, self.tgt),
                    )
                except Exception as e:
                    printlog(
                        '[Error] "%s" getyoutubechatcontinuation %s: %s'
                        % (self.name, self.tgt, e)
                    )
                    writelog(
                        self.logpath,
                        '[Error] "%s" getyoutubechatcontinuation %s: %s'
            ...
            if self.continuation:
                try:
                    chatlist, self.continuation = getyoutubechatlist(
                    ...
                    printlog(
                        '[Error] "%s" getyoutubechatlist %s: %s'
                        % (self.name, self.continuation, e)
                    )
                    writelog(
                        self.logpath,
                        '[Error] "%s" getyoutubechatlist %s: %s'
    ...
    def push(self, chat):
        writelog(
            ...
            "%s\t%s\t%s\t%s\t%s"
            % (
                chat["chat_timestamp_float"],
                chat["chat_username"],
                chat["chat_userchannel"],
                chat["chat_type"],
                chat["chat_text"],
            ),
        )
        ...
        while not self.stop_now:
            ...
            try:
                ...
                if self.is_firstrun:
                    writelog(
                        self.logpath,
                        '[Info] "%s" getyoutubepostdic %s: %s'
                        % (self.name, self.tgt, postdic_new),
                    )
                    self.is_firstrun = False
                writelog(
                    self.logpath,
                    '[Success] "%s" getyoutubepostdic %s' % (self.name, self.tgt),
                )
            except Exception as e:
                printlog(
                    '[Error] "%s" getyoutubepostdic %s: %s' % (self.name, self.tgt, e)
                )
                writelog(
                    self.logpath,
                    '[Error] "%s" getyoutubepostdic %s: %s' % (self.name, self.tgt, e),
        ...
        if pushcolor_dic:
            pushtext = (
                ...
                % (
                    self.__class__.__name__,
                    self.tgt_name,
                    postdic[post_id]["post_text"][0:3000],
                    postdic[post_id]["post_time"],
                    post_id,
                )
            )
            pushall(pushtext, pushcolor_dic, self.push_list)
        ...
        while not self.stop_now:
            ...
            if not self.token:
                try:
                    ...
                    writelog(
                        self.logpath,
                        '[Info] "%s" getyoutubetoken %s: %s'
                        % (self.name, self.tgt, self.token),
                    )
                    writelog(
                        self.logpath,
                        '[Success] "%s" getyoutubetoken %s' % (self.name, self.tgt),
                    )
                except Exception as e:
                    printlog(
                        '[Error] "%s" getyoutubetoken %s: %s' % (self.name, self.tgt, e)
                    )
                    writelog(
                        self.logpath,
                        '[Error] "%s" getyoutubetoken %s: %s'
                        % (self.name, self.tgt, e),
                    )
                    time.sleep(5)
                    continue
            ...
            if self.token:
                try:
                    ...
                    if self.is_firstrun:
                        ...
                        writelog(
                            self.logpath,
                            '[Info] "%s" getyoutubenotedic %s: %s'
                            % (self.name, self.tgt, notedic_new),
                        )
                        self.is_firstrun = False
                    else:
                    ...
                    writelog(
                        self.logpath,
                        '[Success] "%s" getyoutubenotedic %s' % (self.name, self.tgt),
                    )
                except Exception as e:
                    printlog(
                        '[Error] "%s" getyoutubenotedic %s: %s'
                        % (self.name, self.tgt, e)
                    )
                    writelog(
                        self.logpath,
        ...
        if pushcolor_dic:
            pushtext = (
                ...
                % (
                    self.__class__.__name__,
                    self.tgt_name,
                    notedic[note_id]["note_text"],
                    notedic[note_id]["note_time"],
                    notedic[note_id]["note_videoid"],
                )
            )
            pushall(pushtext, pushcolor_dic, self.push_list)
            printlog(
        ...
        while not self.stop_now:
            ...
            try:
                ...
                if self.is_firstrun:
                    ...
                    writelog(
                        self.logpath,
                        '[Info] "%s" gettwitteruser %s: %s'
                        % (self.name, self.tgt, user_datadic_new),
                    )
                    self.is_firstrun = False
                else:
                    pushtext_body = ""
                ...
                writelog(
                    self.logpath,
                    '[Success] "%s" gettwitteruser %s' % (self.name, self.tgt),
                )
            except Exception as e:
                printlog(
                    '[Error] "%s" gettwitteruser %s: %s' % (self.name, self.tgt, e)
                )
                writelog(
                    self.logpath,
                    '[Error] "%s" gettwitteruser %s: %s' % (self.name, self.tgt, e),
...
class TwitterTweet(SubMonitor):
    ...
    def run(self):
        while not self.stop_now:
            ...
            if not self.tgt_restid:
                try:
                    ...
                    writelog(
                        self.logpath,
                        '[Info] "%s" gettwitteruser %s: %s'
                        % (self.name, self.tgt, self.tgt_restid),
                    )
                    writelog(
                        self.logpath,
                        '[Success] "%s" gettwitteruser %s' % (self.name, self.tgt),
            ...
            if self.tgt_restid:
                try:
                    tweetdic_new = gettwittertweetdic(
                    ...
                    if self.is_firstrun:
                        ...
                        writelog(
                            self.logpath,
                            '[Info] "%s" gettwittertweetdic %s: %s'
                            % (self.name, self.tgt, tweetdic_new),
                        )
                        self.is_firstrun = False
                    else:
                    ...
                    writelog(
                        self.logpath,
                        '[Success] "%s" gettwittertweetdic %s'
                        % (self.name, self.tgt_restid),
                    )
                except Exception as e:
                    printlog(
                        '[Error] "%s" gettwittertweetdic %s: %s'
                        % (self.name, self.tgt_restid, e)
                    )
                    writelog(
                        self.logpath,
                        '[Error] "%s" gettwittertweetdic %s: %s'
    ...
    def push(self, tweet_id, tweetdic):
        ...
        pushcolor_vipdic = getpushcolordic(
            "%s\n%s" % (self.tgt, tweetdic[tweet_id]["tweet_mention"]), self.vip_dic
        )
        pushcolor_worddic = getpushcolordic(
            tweetdic[tweet_id]["tweet_text"], self.word_dic
        )
        pushcolor_dic = addpushcolordic(pushcolor_vipdic, pushcolor_worddic)
        ...
        if pushcolor_dic:
            pushtext = (
                ...
                % (
                    self.__class__.__name__,
                    self.tgt_name,
                    tweetdic[tweet_id]["tweet_type"],
                    tweetdic[tweet_id]["tweet_text"],
                    tweetdic[tweet_id]["tweet_media"],
                    tweetdic[tweet_id]["tweet_urls"],
                    datetime.datetime.utcfromtimestamp(
                        tweetdic[tweet_id]["tweet_timestamp"]
                    )
                    .replace(tzinfo=datetime.timezone.utc)
                    .strftime("%Y-%m-%d %H:%M:%S %Z"),
                    self.tgt,
        ...
        while not self.stop_now:
            ...
            try:
                ...
                if self.is_firstrun:
                    ...
                    writelog(
                        self.logpath,
                        '[Info] "%s" gettwittersearchdic %s: %s'
                        % (self.name, self.tgt, tweetdic_new),
                    )
                    self.is_firstrun = False
                else:
                ...
                writelog(
                    self.logpath,
                    '[Success] "%s" gettwittersearchdic %s' % (self.name, self.tgt),
                )
            except Exception as e:
                printlog(
                    '[Error] "%s" gettwittersearchdic %s: %s' % (self.name, self.tgt, e)
                )
                writelog(
                    self.logpath,
                    '[Error] "%s" gettwittersearchdic %s: %s'
                    % (self.name, self.tgt, e),
                )
            time.sleep(self.interval)
                ...
                if url.count("https://youtu.be/"):
                    if (
                        getyoutubevideostatus(
                            url.replace("https://youtu.be/", ""), self.proxy
                        )
                        == "开始"
                    ):
                        is_live = True
...
class TwitcastLive(Monitor):
    ...
    def run(self):
        while not self.stop_now:
            ...
            try:
                ...
                writelog(
                    self.logpath,
                    '[Success] "%s" gettwitcastlive %s' % (self.name, self.tgt),
                )
            except Exception as e:
                printlog(
                    '[Error] "%s" gettwitcastlive %s: %s' % (self.name, self.tgt, e)
                )
                writelog(
                    self.logpath,
                    '[Error] "%s" gettwitcastlive %s: %s' % (self.name, self.tgt, e),
    ...
    def push(self, live_id):
        if self.status_push.count(self.livedic[live_id]["live_status"]):
            pushcolor_vipdic = getpushcolordic(self.tgt, self.vip_dic)
            pushcolor_worddic = getpushcolordic(
                self.livedic[live_id]["live_title"], self.word_dic
            )
            pushcolor_dic = addpushcolordic(pushcolor_vipdic, pushcolor_worddic)
    ...
    def run(self):
        while not self.stop_now:
            ...
            try:
                ...
                printlog(
                    '[Error] "%s" gettwitcastchatlist %s: %s'
                    % (self.name, self.chat_id_old, e)
                )
                writelog(
                    self.logpath,
                    '[Error] "%s" gettwitcastchatlist %s: %s'
    ...
    def run(self):
        while not self.stop_now:
            ...
            try:
                ...
                writelog(
                    self.logpath,
                    '[Success] "%s" getfanboxuser %s' % (self.name, self.tgt),
                )
            except Exception as e:
                printlog('[Error] "%s" getfanboxuser %s: %s' % (self.name, self.tgt, e))
                writelog(
                    self.logpath,
                    '[Error] "%s" getfanboxuser %s: %s' % (self.name, self.tgt, e),
                )
            time.sleep(self.interval)
        ...
        while not self.stop_now:
            ...
            try:
                ...
                if self.is_firstrun:
                    writelog(
                        self.logpath,
                        '[Info] "%s" getfanboxpostdic %s: %s'
                        % (self.name, self.tgt, postdic_new),
                    )
                    self.is_firstrun = False
                writelog(
                    self.logpath,
                    '[Success] "%s" getfanboxpostdic %s' % (self.name, self.tgt),
                )
            except Exception as e:
                printlog(
                    '[Error] "%s" getfanboxpostdic %s: %s' % (self.name, self.tgt, e)
                )
                writelog(
                    self.logpath,
                    '[Error] "%s" getfanboxpostdic %s: %s' % (self.name, self.tgt, e),
        ...
        if pushcolor_dic:
            pushtext = (
                ...
                % (
                    self.__class__.__name__,
                    self.tgt_name,
                    postdic[post_id]["post_title"],
                    postdic[post_id]["post_text"][0:2500],
                    postdic[post_id]["post_type"],
                    postdic[post_id]["post_fee"],
                    datetime.datetime.utcfromtimestamp(
                        postdic[post_id]["post_publishtimestamp"]
                    )
                    .replace(tzinfo=datetime.timezone.utc)
                    .strftime("%Y-%m-%d %H:%M:%S %Z"),
                    self.tgt,
    ...
    def run(self):
        ...
        while not self.stop_now:
            ...
            try:
                ...
                writelog(
                    self.logpath,
                    '[Success] "%s" getbilibililivedic %s' % (self.name, self.tgt),
                )
            except Exception as e:
                printlog(
                    '[Error] "%s" getbilibililivedic %s: %s' % (self.name, self.tgt, e)
                )
                writelog(
                    self.logpath,
                    '[Error] "%s" getbilibililivedic %s: %s' % (self.name, self.tgt, e),
            ...
            if pushcolor_dic:
                pushtext = (
                    ...
                    % (
                        ...
                        self.livedic[live_id]["live_status"],
                        self.livedic[live_id]["live_title"],
                        datetime.datetime.utcfromtimestamp(live_id)
                        .replace(tzinfo=datetime.timezone.utc)
                        .strftime("%Y-%m-%d %H:%M:%S %Z"),
                        self.tgt,
                    )
...
class BilibiliChat(SubMonitor):
    ...
    def on_message(self, message):
        ...
        for packet in packetlist:
            if packet["operation"] == 8:
                self.is_linked = True
                writelog(
                    self.logpath, '[Success] "%s" connected %s' % (self.name, self.tgt)
                )
    ...
    def on_error(self, error):
        writelog(
            self.logpath, '[Error] "%s" error %s: %s' % (self.name, self.tgt, error)
        )
        ...
        while not self.stop_now:
            if self.hostcount < len(self.hostlist):
                ...
                try:
                    ...
                    writelog(
                        self.logpath,
                        '[Info] "%s" getbilibilichathostlist %s: %s'
                        % (self.name, self.tgt, self.hostlist),
                    )
                    writelog(
                        self.logpath,
                        '[Success] "%s" getbilibilichathostlist %s'
                        % (self.name, self.tgt),
                    )
                except Exception as e:
                    printlog(
                        '[Error] "%s" getbilibilichathostlist %s: %s'
                        % (self.name, self.tgt, e)
                    )
                    writelog(
                        self.logpath,
            ...
            try:
                ...
                if self.is_firstrun:
                    ...
                    if (
                        ...
                        pushtext = (
                            ...
                            % (
                                ...
                                self.tgt_name,
                                datetime.datetime.utcfromtimestamp(
                                    user_datadic_new["user_gametimestamp"]
                                )
                                .replace(tzinfo=datetime.timezone.utc)
                                .strftime("%Y-%m-%d %H:%M:%S %Z"),
                                self.tgt_region,
                    ...
                    for key in user_datadic_new:
                        ...
                        # 其他 不推送
                            ...
                            self.userdata_dic[key] = user_datadic_new[key]
                writelog(
                    self.logpath, '[Success] "%s" getloluser %s' % (self.name, self.tgt)
                )
                ...
                if (
                    ...
                    try:
                        ...
                        writelog(
                            self.logpath,
                            '[Success] "%s" renewloluser %s'
                            % (self.name, self.userdata_dic["user_id"]),
                        )
                    except Exception as e:
                        printlog(
                            '[Error] "%s" renewloluser %s: %s'
                            % (self.name, self.userdata_dic["user_id"], e)
                        )
                        writelog(
                            self.logpath,
                            '[Error] "%s" renewloluser %s: %s'
                ...
                writelog(
                    self.logpath,
                    '[Error] "%s" getloluser %s: %s' % (self.name, self.tgt, e),
                )
            time.sleep(self.interval)
    ...
    def run(self):
        while not self.stop_now:
            ...
            try:
                ...
                writelog(
                    self.logpath,
                    '[Success] "%s" getsteamuser %s' % (self.name, self.tgt),
                )
            except Exception as e:
                printlog('[Error] "%s" getsteamuser %s: %s' % (self.name, self.tgt, e))
                writelog(
                    self.logpath,
                    '[Error] "%s" getsteamuser %s: %s' % (self.name, self.tgt, e),
                )
            time.sleep(self.interval)
            ...
            try:
                ...
                if self.is_firstrun:
                                ...
                                if gameid > self.lastgameid:
                                    pushtext = (
                                        ...
                                        % (
                                            self.__class__.__name__,
                                            self.tgt_name,
                                            user_datadic_new["user_gamedic"][gameid][
                                                "game_type"
                                            ],
                                            user_datadic_new["user_gamedic"][gameid][
                                                "game_result"
                    ...
                    if pushtext_body:
                        ...
                        self.push(pushtext)
                writelog(
                    self.logpath, '[Success] "%s" getosuuser %s' % (self.name, self.tgt)
                )
            except Exception as e:
                printlog('[Error] "%s" getosuuser %s: %s' % (self.name, self.tgt, e))
                writelog(
                    self.logpath,
                    '[Error] "%s" getosuuser %s: %s' % (self.name, self.tgt, e),
                )
            time.sleep(self.interval)
...
def getyoutubevideodic(user_id, cookies, proxy):
    try:
        ...
        videolist_json = json.loads(
            re.findall('window\["ytInitialData"\] = (.*);', response.text)[0]
        )
        videolist = []
        ...
        for video_json in videolist:
            ...
            if "publishedTimeText" in video_json:
                ...
                if (
                    status.count("首播")
                    or status.count("PREMIERE")
                    or status.count("プレミア")
...
def getyoutubevideostatus(video_id, cookies, proxy):
    ...
    try:
        ...
        soup = BeautifulSoup(response.text, "lxml")
        script = eval(
            '"""{}"""'.format(soup.find(string=re.compile(r'\\"isLiveContent\\":')))
        )
        if script == "None":
            video_status = "删除"
    ...
    try:
        ...
        if "stopHeartbeat" in response.json():
            ...
            if response.json()["playabilityStatus"]["status"] == "UNPLAYABLE":
                ...
                "liveStreamability" not in response.json()["playabilityStatus"]
                or "displayEndscreen"
                in response.json()["playabilityStatus"]["liveStreamability"][
...
def getyoutubechatlist(continuation, proxy):
    try:
        ...
        headers = {
            ...
            "x-spf-referer": "https://www.youtube.com/live_chat?continuation="
            + continuation,
            "x-youtube-utc-offset": "480",
            "x-youtube-client-name": "1",
            "x-spf-previous": "https://www.youtube.com/live_chat?continuation="
            + continuation,
        ...
        response = requests.get(
            url, headers=headers, params=params, timeout=(3, 7), proxies=proxy
        )
        continuation_new = re.findall('"continuation":"([^"]*)"', response.text)[0]
        chatlist_json = json.loads(response.text)["response"]["continuationContents"][
                ...
                if "addChatItemAction" in chat:
                    ...
                    if chat_dic:
                        chat_timestamp_float = (
                            float(chat_dic["timestampUsec"]) / 1000000
                        )
                        ...
                        if "purchaseAmountText" in chat_dic:
                            chat_type += (
                                " %s" % chat_dic["purchaseAmountText"]["simpleText"]
                            )
                        chatlist.append(
                            {
                                "chat_timestamp_float": chat_timestamp_float,
...
def getyoutubetoken(cookies, proxy):
    try:
        ...
        response = requests.get(
            "https://www.youtube.com", headers=headers, cookies=cookies, proxies=proxy
        )
        ...
        if "multiPageMenuNotificationSectionRenderer" in notesec_json:
            for note in notesec_json["multiPageMenuNotificationSectionRenderer"][
                ...
                if "notificationRenderer" in note:
                    ...
                    note_videoid = note["notificationRenderer"]["navigationEndpoint"][
                        "commandMetadata"
                    ]["webCommandMetadata"]["url"].replace("/watch?v=", "")
...
def gettwitteruser(user_screenname, cookies, proxy):
    try:
        ...
        params = (
            (
                "variables",
                '{"screen_name":"%s","withHighlightedLabel":false}' % user_screenname,
            ),
        )
        response = requests.get(
...
def gettwittertweetdic(user_restid, cookies, proxy):
    try:
        ...
        response = requests.get(
            "https://api.twitter.com/2/timeline/profile/%s.json" % user_restid,
            headers=headers,
            params=params,
            cookies=cookies,
            timeout=(3, 7),
...
def gettwitcastchatlist(live_id, proxy):
    try:
        twitcastchatlist = []
        url = (
            "https://twitcasting.tv/userajax.php?c=listall&m=%s&n=10&f=0k=0&format=json"
            % live_id
        )
        response = requests.get(url, timeout=(3, 7), proxies=proxy)
...
def getfanboxuser(user_id, proxy):
    try:
        ...
        response = requests.get(
            "https://api.fanbox.cc/creator.get?creatorId=%s" % user_id,
            headers=headers,
            timeout=(3, 7),
            proxies=proxy,
        )
...
def getfanboxpostdic(user_id, cookies, proxy):
    try:
        ...
        response = requests.get(
            "https://api.fanbox.cc/post.listCreator?creatorId=%s&limit=10" % user_id,
            headers=headers,
            cookies=cookies,
            timeout=(3, 7),
            proxies=proxy,
...
def getbilibililivedic(room_id, proxy):
    try:
        live_dic = {}
        response = requests.get(
            "http://api.live.bilibili.com/room/v1/Room/get_info?room_id=%s" % room_id,
            timeout=(3, 7),
            proxies=proxy,
        )
...
def getbilibilichathostlist(proxy):
    ...
    try:
        ...
        for hostserver in hostserver_list:
            hostlist.append(
                "wss://%s:%s/sub" % (hostserver["host"], hostserver["wss_port"])
            )
        if hostlist:
            return hostlist
        else:
...
def getloluser(user_name, user_region, proxy):
    try:
        userdata_dic = {}
        response = requests.get(
            "https://%s.op.gg/summoner/l=en_US&userName=%s" % (user_region, user_name),
            timeout=(3, 7),
            proxies=proxy,
        )
        soup = BeautifulSoup(response.text, "lxml")
        ...
        for gameitem in soup.find_all(class_="GameItemWrap"):
            ...
            game_kda = "%s/%s/%s" % (
                gameitem.find(class_="Kill").text,
                gameitem.find(class_="Death").text,
        ...
        response = requests.get(
            "https://%s.op.gg/summoner/spectator/l=en_US&userName=%s"
...
def renewloluser(user_id, user_region, proxy):
    try:
        ...
        response = requests.post(
            "https://%s.op.gg/summoner/ajax/renew.json/" % user_region,
            headers=headers,
            data=data,
            timeout=(3, 7),
            proxies=proxy,
...
def getsteamuser(user_id, cookies, proxy):
    try:
        userdata_dic = {}
        response = requests.get(
            "https://steamcommunity.com/profiles/%s" % user_id,
            cookies=cookies,
            timeout=(3, 7),
            proxies=proxy,
        )
        ...
        if not soup.find(class_="profile_private_info"):
            ...
            for item_count in soup.find_all(class_="profile_count_link ellipsis"):
                userdata_dic[
                    "user_" + item_count.find(class_="count_link_label").text.strip()
...
def getosuuser(user_id, cookies, proxy):
    try:
        response = requests.get(
            "https://osu.ppy.sh/users/%s" % user_id,
            cookies=cookies,
            timeout=(3, 7),
            proxies=proxy,
        )
...
def pushtoall(pushtext, push):
    ...
    if push["type"] == "qq_user":
        ...
        # 带文字推送可能导致语音和短信提醒失效
        url = "https://miaotixing.com/trigger?id=%s&text=%s" % (
            push["id"],
            quote(str(pushtext)),
        )
        pushtourl(url)
